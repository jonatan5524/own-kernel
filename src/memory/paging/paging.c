...
struct paging_4gb_chunk *paging_new_4gb(uint8_t flags) {
  uint32_t *directory = (uint32_t *)kernel_zalloc(
      sizeof(uint32_t) * PAGING_TOTAL_ENTRIES_PER_TABLE);
  int offset = 0;

  for (int i = 0; i < PAGING_TOTAL_ENTRIES_PER_TABLE; i++) {
    uint32_t *entry = (uint32_t *)kernel_zalloc(sizeof(uint32_t) *
  ...
  struct paging_4gb_chunk *chunk_4gb =
      (struct paging_4gb_chunk *)kernel_zalloc(sizeof(struct paging_4gb_chunk));
...
int paging_map(struct paging_4gb_chunk *directory, void *virtual_address,
               void *physical_address, int flags) {
  if (((unsigned int)virtual_address % PAGING_PAGE_SIZE) ||
      ((unsigned int)physical_address % PAGING_PAGE_SIZE)) {
    return -INVALID_ARGUMENT_ERROR;
  }

  return paging_set_physical_address(directory->directory_entry,
                                     virtual_address,
                                     (uint32_t)physical_address | flags);
}

int paging_map_range(struct paging_4gb_chunk *directory, void *virtual_address,
                     void *physical_address, int total_pages, int flags) {
  int res = 0;
...
int paging_map_to(struct paging_4gb_chunk *directory, void *virtual_address,
                  void *physical_address, void *physical_end_address,
                  int flags) {
  int res = 0;
  ...
  res = paging_map_range(directory, virtual_address, physical_address,
                         total_pages, flags);
...
int paging_set_physical_address(uint32_t *directory, void *virtual_address,
                                uint32_t val) {
  if (!paging_is_aligned(virtual_address)) {
    return -INVALID_ARGUMENT_ERROR;
  }
...
void *paging_get_physical_address(uint32_t *directory, void *virtual_address) {
  void *virtual_address_new =
      (void *)paging_align_to_lower_page(virtual_address);
  void *difference =
      (void *)((uint32_t)virtual_address - (uint32_t)virtual_address_new);

  return (void *)((paging_get(directory, virtual_address_new) & 0xFFFFF000) +
                  difference);
}
