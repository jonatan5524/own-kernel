...
static int fat16_read_internal_read_stream(struct disk* disk, struct disk_stream* stream, int cluster, int offset, int total, void* out)
{
  int res = 0;
  struct fat_private* private = disk->fs_private;
  int size_of_cluster_bytes = private->header.primary_header.sectors_per_cluster * disk->sector_size;
  ...
  if (total > 0)
  {
    res = fat16_read_internal_read_stream(disk, stream, cluster, offset + total_to_read, total, out + total_to_read);
  }
...
static int fat16_read_internal(struct disk* disk, int starting_cluster, int offset, int total, void* out)
{
  ...
  return fat16_read_internal_read_stream(disk, stream, starting_cluster, offset, total, out);
}
